...
parser.add_argument(
    "--wham_dir", type=str, required=True, help="Path to wham_noise root directory"
)
parser.add_argument(
    "--metadata_dir",
    type=str,
...
parser.add_argument(
    "--n_src", type=int, required=True, help="Number of sources in mixtures"
)
parser.add_argument(
    "--freqs",
    nargs="+",
...
def create_librimix(
    librispeech_dir, wham_dir, out_dir, metadata_dir, freqs, n_src, modes, types
):
    ...
    for md_filename in md_filename_list:
        ...
        process_metadata_file(
            csv_path, freqs, n_src, librispeech_dir, wham_dir, out_dir, modes, types
        )
...
def process_metadata_file(
    ...
    for freq in freqs:
        ...
        for mode in modes:
            ...
            print(f"Creating mixtures and sources from {csv_path} " f"in {dir_path}")
            # Create subdir
            if types == ["mix_clean"]:
...
def process_utterance(
    ...
    mix_id, gain_list, sources, speakers, texts, profiles = read_sources(
        row, n_src, librispeech_dir, wham_dir
    )
    # Transform sources
    transformed_sources, lengths = transform_sources(sources, freq, mode, gain_list)
...
def create_empty_mixture_md(n_src, subdir):
    ...
    if subdir == "mix_clean":
        ...
        for i in range(n_src):
            mixture_dataframe[f"source_{i + 1}_speaker"] = {}
        for i in range(n_src):
            mixture_dataframe[f"source_{i + 1}_text"] = {}
        for i in range(n_src):
            mixture_dataframe[f"source_{i + 1}_length"] = {}
        for i in range(n_src):
            mixture_dataframe[f"source_{i + 1}_profiles"] = {}
    elif subdir == "mix_both":
        for i in range(n_src):
            mixture_dataframe[f"source_{i + 1}_path"] = {}
        for i in range(n_src):
        ...
        mixture_dataframe[f"source_1_speaker"] = {}
        mixture_dataframe[f"source_1_text"] = {}
        mixture_dataframe[f"source_1_length"] = {}
        mixture_dataframe[f"source_1_profiles"] = {}
        mixture_dataframe[f"noise_path"] = {}
...
def read_sources(row, n_src, librispeech_dir, wham_dir):
    ...
    sources_list, speakers_list, texts_list, profiles_list = [], [], [], []
    max_length = 0

    # Read the files to make the mixture
    for sources_path in sources_path_list:
        sources_path = Path(os.path.join(librispeech_dir, sources_path))
        trans_dir = sources_path.parent
        source_id = sources_path.stem

        with open(
            trans_dir / ("-".join(source_id.split("-")[:-1]) + ".trans.txt"), "r"
        ) as f:
            for line in f.readlines():
                recording_id, text = line.strip().split(maxsplit=1)
                if recording_id == source_id:
                    break

        source, _ = sf.read(sources_path, dtype="float32")
        speaker = source_id.split("-")[0]
        profiles = ";".join([str(p.absolute()) for p in trans_dir.glob("*.flac") if p.stem != source_id])
        # Get max_length
    ...
    return mixture_id, gain_list, sources_list, speakers_list, texts_list, profiles_list
...
def transform_sources(sources_list, freq, mode, gain_list):
    ...
    # Reshape sources
    reshaped_sources, lengths = fit_lengths(sources_list_resampled, mode)
    return reshaped_sources, lengths
...
def fit_lengths(source_list, mode):
    ...
    if mode == "min":
        ...
        for source in source_list:
            ...
            original_lengths.append(len(source))
    else:
        ...
        for source in source_list:
            ...
            original_lengths.append(len(source))
    return sources_list_reshaped, original_lengths
